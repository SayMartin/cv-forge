  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <Field label={d.fields.subtitle.label} value={form.subtitle} onChange={(v) => set("subtitle", v)} placeholder={d.fields.subtitle.placeholder} />
        <Field label={d.fields.date.label} value={form.date} onChange={(v) => set("date", v)} placeholder={d.fields.date.placeholder} />
    ...
    <div className="space-y-4">
      {!creating ? (
        ...
        <div className="bg-white border border-(--cl-accent) rounded-xl px-5 py-4">
          ...
          <OtherForm initial={EMPTY_FORM} submitLabel={t.create} onSubmit={handleCreate} onCancel={() => setCreating(false)} />
        </div>
      )}
        ...
        <div key={item.id} className="bg-white border border-(--cl-border) rounded-xl px-5 py-4 space-y-3">
          {editingId === item.id ? (
            <>
              ...
              <OtherForm initial={itemToForm(item)} submitLabel={t.save} onSubmit={(form) => handleUpdate(item.id, form)} onCancel={() => setEditingId(null)} />
            </>
          ) : (
