...
│       ├── skill-category-seed.ts                      ← seeded categories: stable keys + per-locale names. Prisma-free; seed data, NOT a dictionary
│       ├── skill-categories.ts                         ← the seeding write + signUpLocale() (user.locale, else the cookie on the creating request)
...
#### Seeding: the names are data, the keys are code

Every new account is seeded from `src/lib/skill-category-seed.ts` by the Better Auth `user.create.after` hook, **in the language the person is signing up in**. `MAX_SKILL_CATEGORIES` is derived as `SEED_CATEGORY_COUNT + 1` rather than written down, so a new user can always add a category of their own without deleting a seeded one and the guard cannot drift from the list it guards.

The seeded set is keyed by a stable `SeedCategoryKey` — `programming`, `devops`, `language` — with a `Record<Locale, Record<SeedCategoryKey, string>>` of display names. Two things follow:

- **`kind` derives from the key, never from the English name.** The seed function used to compare `name === "Language"`, which was the one place in the codebase contradicting what `kind` is *for*. A Swedish account seeds "Språk" and still gets `kind: "language"`.
- **These names are not in `src/i18n/dictionaries/`, and must not move there.** They are written to `skill_category` rows once and belong to the user from that moment — renameable, reorderable, deletable. A dictionary is re-read on every render, so putting them there would imply that editing a translation renames everyone's rows.

**Existing accounts are not touched.** There is no backfill and no migration: an account seeded in English keeps its English rows, because by then they are that person's data and may already have been customised. Only new sign-ups get their own language.

Where the sign-up language comes from is two questions, because the two paths know different things. Email/password passes `locale` in the same call that creates the row (`input: true`), so `user.locale` is set. Google OAuth never touches the sign-up form, so `user.locale` is NULL there — and the language has to come from the `cvforge_locale` cookie on the callback request, which `signUpLocale()` parses out of the hook's endpoint context. Without that second step every Swedish account created with Google would start with eight English categories to rename.
...
#### The importer reads the user's categories, not a constant

`POST /api/cv-import` builds **both** the model's `category` enum and its category prompt from `skillCategory.findMany({ where: { userId } })`. It used to constrain Gemini to a hardcoded English list and then resolve the answer against the user's rows — which works only while the two agree.

That was already a live bug for anyone who renamed a category, and per-locale seeding would have made it the default for half the users: of the eight seeded names, **only "Backend" and "Frontend" are spelled the same in both languages, so six of eight categories' skills would have imported uncategorised.** Silently, because an unmatched name is a `null` categoryId rather than an error — the kind of regression that takes months to notice.

The disambiguation rules in the prompt are the ones that actually change the output — without the first, "Python" reliably lands under a spoken-languages heading — so they are carried across renames and translations rather than dropped:

- The spoken-languages rule finds its category by **`kind === "language"`**, which holds even for a category renamed to something no locale seeds.
- The Backend/Frontend, Tools, and DevOps/Cloud rules are emitted only when the categories they name are still present, matched by **seeded key across every locale** (an account seeded in English and later switched to Swedish still holds English rows, and both must be recognised). A user-invented or heavily renamed category matches nothing and is simply offered to the model on its own merits — the right amount to say about a heading only they understand.
- An account with no categories at all gets a plain optional string and an instruction to omit the field; every skill imports uncategorised, which is the only honest outcome when there is nowhere to put one.

The prompt itself stays English. It is an instruction to the model, not text anyone reads; what has to be in the user's language are the category *names* it quotes, and those come from their own rows.
...
- [x] Phase 37 — **Bilingual (Swedish/English).** The whole app moved under `app/[lang]`, with `proxy.ts` negotiating `Accept-Language` on a first visit and a flag toggle in the navbar. Language lives in three places that answer three different questions — the URL, the `cvforge_locale` cookie, and `User.locale` — and a fourth, `Cv.language`, that is a different axis entirely: a Swedish speaker applying abroad keeps a Swedish interface and exports an English CV. Two dictionaries, because functions cannot cross the server→client boundary: `dictionaries/{en,sv}` is serialised to the client, `dictionaries/server` holds the two Resend templates. English is the contract in both (`type Dictionary = typeof en`, never `as const`), so a missing Swedish key is a compile error in the file where the fix belongs. API failures travel as `ApiErrorCode`s and are translated on the client, because a Route Handler cannot read `next/root-params` and so cannot know the caller's language. Every metadata site became a `generateMetadata`; hreflang is emitted on the two URLs a crawler can actually read, and `/privacy` canonicalises to `/en/privacy` because its prose is English in both locales. Skill categories are seeded per locale from stable keys, and the PDF importer now builds its enum and its prompt from the user's own rows rather than a hardcoded English list — a bug that already broke imports for anyone who renamed a category, and would have left six of eight categories uncategorised for every Swedish account.
